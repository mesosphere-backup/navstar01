...
handle_info({timeout, Ref, kill}, #state{ref=Ref, reductions=Prev}=State) ->
    ...
    Timeout = application:get_env(navstar, killer_timeout, timer:minutes(5)),
...
kill(Pid, MFA) ->
    ...
    case application:get_env(navstar, enable_killer, true) of
        true -> exit(Pid, kill);
        false -> true
    end.
...
stuck_fun({gen_event, fetch_msg, 5}) -> false;
