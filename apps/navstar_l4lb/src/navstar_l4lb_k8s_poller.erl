...
%% @doc
%% Polls K8s apiserver if {navstar_l4lb, agent_polling_enabled} is true
%%
%% @end
%% Created : 25 Oct 2017 9:14 AM
%%-------------------------------------------------------------------

-module(navstar_l4lb_k8s_poller).
-author("dgoel").
...
fetch_pods(URI, Selector) ->
    ...
    QueryParams0 = [string:join([binary_to_list(Key), binary_to_list(maps:get(Key, Selector))], "=") || Key <- SelectorKeys],
    QueryParams1 = string:join(QueryParams0, ","),
    QueryParams2 = string:join(["labelSelector", QueryParams1], "="),
    case navstar_l4lb_k8s_client:get(URI, Path, QueryParams2) of
        {error, Reason} ->
