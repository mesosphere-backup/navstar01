...
update_netns(add_netns, UpdateValue, State = #state{ns = Namespaces0, ipvs_mgr = IPVSMgr, route_mgr = RouteMgr}) ->
    Namespaces1 = navstar_l4lb_route_mgr:add_netns(RouteMgr, UpdateValue),
    Namespaces1 = navstar_l4lb_ipvs_mgr:add_netns(IPVSMgr, UpdateValue),
...
update_netns(remove_netns, UpdateValue, State = #state{ns = Namespaces0, ipvs_mgr = IPVSMgr, route_mgr = RouteMgr}) ->
    Namespaces1 = navstar_l4lb_route_mgr:remove_netns(RouteMgr, UpdateValue),
    Namespaces1 = navstar_l4lb_route_mgr:remove_netns(IPVSMgr, UpdateValue),
...
push_routes(Namespace, #state{routes = Routes, route_mgr = RouteMgr}) ->
    navstar_l4lb_route_mgr:add_routes(RouteMgr, Routes, Namespace).
...
installed_routes(Namespace, #state{route_mgr = RouteMgr}) ->
    navstar_l4lb_route_mgr:get_routes(RouteMgr, Namespace).
...
do_update_routes(NewRoutes, OldRoutes, Namespace, #state{route_mgr = RouteMgr}) -> 
    ...
    navstar_l4lb_route_mgr:add_routes(RouteMgr, RoutesToAdd, Namespace),
    navstar_l4lb_route_mgr:remove_routes(RouteMgr, RoutesToDel, Namespace).
...
installed_state(Namespace, #state{ipvs_mgr = IPVSMgr}) ->
    Services = navstar_l4lb_ipvs_mgr:get_services(IPVSMgr, Namespace),
    ServicesAndDests0 = [{Service, navstar_l4lb_ipvs_mgr:get_dests(IPVSMgr, Service, Namespace)} || Service <- Services],
...
normalize_services_and_dests({Service0, Destinations0}) ->
    {AddressFamily, Service1} = navstar_l4lb_ipvs_mgr:service_address(Service0),
    Destinations1 = lists:map(
                      fun(Dest) ->  
                          navstar_l4lb_ipvs_mgr:destination_address(AddressFamily, Dest) 
                      end, Destinations0),
...
modify_service({Protocol, IP, Port}, BEAdd, BERemove, Namespace, #state{ipvs_mgr = IPVSMgr}) ->
    lists:foreach(fun
        ({_AgentIP, {BEIP, BEPort}}) ->
            navstar_l4lb_ipvs_mgr:add_dest(IPVSMgr, IP, Port, BEIP, BEPort, Protocol, Namespace);
        ({BEIP, BEPort}) ->
            navstar_l4lb_ipvs_mgr:add_dest(IPVSMgr, IP, Port, BEIP, BEPort, Protocol, Namespace)
        end,
        BEAdd),
    lists:foreach(fun
        ({_AgentIP, {BEIP, BEPort}}) ->
            navstar_l4lb_ipvs_mgr:remove_dest(IPVSMgr, IP, Port, BEIP, BEPort, Protocol, Namespace);
        ({BEIP, BEPort}) ->
            navstar_l4lb_ipvs_mgr:remove_dest(IPVSMgr, IP, Port, BEIP, BEPort, Protocol, Namespace)
        end,
        BERemove).
...
remove_service({Protocol, IP, Port}, Namespace, #state{ipvs_mgr = IPVSMgr}) ->
    navstar_l4lb_ipvs_mgr:remove_service(IPVSMgr, IP, Port, Protocol, Namespace).

add_service({Protocol, IP, Port}, BEs, Namespace, #state{ipvs_mgr = IPVSMgr}) ->
    navstar_l4lb_ipvs_mgr:add_service(IPVSMgr, IP, Port, Protocol, Namespace),
    lists:foreach(fun
      ({_AgentIP, {BEIP, BEPort}}) ->
          navstar_l4lb_ipvs_mgr:add_dest(IPVSMgr, IP, Port, BEIP, BEPort, Protocol, Namespace);
      ({BEIP, BEPort}) ->
