-module(navstar_l4lb_metrics_SUITE).
-compile(export_all).
...
test_gen_server(_Config) ->
    hello = erlang:send(navstar_l4lb_metrics, hello),
    ok = gen_server:call(navstar_l4lb_metrics, hello),
    ok = gen_server:cast(navstar_l4lb_metrics, hello),
    sys:suspend(navstar_l4lb_metrics),
    sys:change_code(navstar_l4lb_metrics, random_old_vsn, navstar_l4lb_metrics, []),
    sys:resume(navstar_l4lb_metrics).

test_push_metrics(_Config) ->
    push_metrics = erlang:send(navstar_l4lb_metrics, push_metrics),
    timer:sleep(2000),
...
set_interval(test_wait_metrics) ->
    application:set_env(navstar_l4lb, metrics_interval_seconds, 1),
    application:set_env(navstar_l4lb, metrics_splay_seconds, 1);
set_interval(_) -> ok.
