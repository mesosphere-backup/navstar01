...
%%%-------------------------------------------------------------------
-module(navstar_overlay_netlink_SUITE).
-author("dgoel").
...
test_ip_cmds(Config) ->
    ...
    {ok, _} = navstar_overlay_netlink:iplink_add(Pid, ?IFNAME, "vxlan", 1026, 64000),
    ...
    MacStr = mac_ntoa(Mac),
    {ok, _} = navstar_overlay_netlink:iplink_set(Pid, Mac, ?IFNAME),
    ...
    Family = inet,
    PrefixLen = 32,
    {ok, _} = navstar_overlay_netlink:ipaddr_replace(Pid, Family, IP, PrefixLen, ?IFNAME),
    ...
    {ok, _} = navstar_overlay_netlink:ipneigh_replace(Pid, Family, IP, Mac, ?IFNAME),
    ...
    DstIP = {192,168,65,91},
    {ok, _} = navstar_overlay_netlink:iproute_replace(Pid, Family, DstIP, 32, IP, 42),
...
test_bridge_fdb_replace(Config) ->
    ...
    {ok, _} = navstar_overlay_netlink:bridge_fdb_replace(Pid, DstIP, DstMac, ?IFNAME),
...
test_iprule_cmds(Config) ->
    ...
    SrcPrefixLen = 8,
    Family = inet,
    {ok, _} = navstar_overlay_netlink:iprule_add(Pid, Family, Src, SrcPrefixLen, 42),
    ...
    %% Test2: ip rule show
    {ok, Rules} = navstar_overlay_netlink:iprule_show(Pid, Family),
    Rule = navstar_overlay_netlink:make_iprule(Family, Src, SrcPrefixLen, 42),
    true = navstar_overlay_netlink:is_iprule_present(Rules, Rule).
