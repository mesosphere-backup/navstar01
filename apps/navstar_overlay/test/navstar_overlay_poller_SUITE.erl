-module(navstar_overlay_poller_SUITE).
...
init_per_testcase(TestCaseName, Config) ->
    ...
    meck:expect(httpc, request, fun(get, _, _, _) ->
        Node = <<"master1@nohost">>,
        Data = navstar_overlay_mockhttp_frontend:create_data(Node),
...
start_get_kill_poller() ->
    ...
    {ok, Rules} =
        try
            {ok, _Rules} = navstar_overlay_netlink:iprule_show(NetlinkPid, inet)
        after
