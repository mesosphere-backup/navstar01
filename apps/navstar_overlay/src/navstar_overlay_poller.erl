...
maybe_create_vtep(_Overlay = #mesos_state_agentoverlayinfo{backend = Backend}) ->
    ...
    case navstar_overlay_helper:run_command("ip link show dev ~s", [VTEPName]) of
        {ok, _Output} ->
            ok;
        {error, 1, _} ->
            {ok, _} = navstar_overlay_helper:run_command("ip link add dev ~s type vxlan id ~B dstport 64000", [VTEPName, VNI])
    end,
    {ok, _} = navstar_overlay_helper:run_command("ip link set ~s address ~s", [VTEPName, VTEPMAC]),
    {ok, _} = navstar_overlay_helper:run_command("ip link set ~s up", [VTEPName]),
    case navstar_overlay_helper:run_command("ip addr add ~s dev ~s", [VTEPIP, VTEPName]) of
        {ok, _} ->
            ok;
...
maybe_add_ip_rule(_Overlay = #mesos_state_agentoverlayinfo{info = #mesos_state_overlayinfo{subnet = Subnet}}) ->
    ...
    {ok, Rules} = navstar_overlay_helper:run_command("ip rule show"),
    case string:str(Rules, RuleStr) of
        0 ->
            {ok, _} = navstar_overlay_helper:run_command("ip rule add from ~s lookup 42", [Subnet]);
        _ ->
            ok
    end.
