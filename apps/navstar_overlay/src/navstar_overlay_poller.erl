...
maybe_create_vtep(_Overlay = #mesos_state_agentoverlayinfo{backend = Backend}, 
  #state{netlink = Pid}) ->
    ...
    {ok, _} = navstar_overlay_netlink:ipaddr_replace(Pid, inet, ParsedVTEPIP, PrefixLen, VTEPNameStr),
    case VTEPIP6 of
      undefined ->
          ok;
      _ ->
          {ParsedVTEPIP6, PrefixLen6} = parse_subnet(VTEPIP6),
           {ok, _} = navstar_overlay_netlink:ipaddr_replace(Pid, inet6, ParsedVTEPIP6, PrefixLen6, VTEPNameStr)
    end. 
...
maybe_add_ip_rule2(Pid, Subnet) ->
    ...
    {ok, Rules} = navstar_overlay_netlink:iprule_show(Pid, inet),
    Rule = navstar_overlay_netlink:make_iprule(inet, ParsedSubnetIP, PrefixLen, ?TABLE),
    case navstar_overlay_netlink:is_iprule_present(Rules, Rule) of
        false ->
            {ok, _} = navstar_overlay_netlink:iprule_add(Pid, inet, ParsedSubnetIP, PrefixLen, ?TABLE);
        _ ->
            ok
    end.
