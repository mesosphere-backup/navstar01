...
maybe_configure_and_reply(LashupEvent, MyPid) ->
    ...
    reply(MyPid, {navstar_overlay_configure, applied_config, LashupEvent}).
...
configure_overlay_entry(Overlay, _VTEPIPPrefix = {VTEPIP, _PrefixLen}, LashupValue) ->
    ...
    {ok, _} = navstar_overlay_helper:run_command("ip neigh replace ~s lladdr ~s dev ~s nud permanent",
        [FormattedVTEPIP, FormattedMAC, VTEPName]),
    %bridge fdb add to 00:17:42:8a:b4:05 dst 192.19.0.2 dev vxlan0
    {ok, _} = navstar_overlay_helper:run_command("bridge fdb replace to ~s dst ~s dev ~s", [FormattedMAC, FormattedAgentIP, VTEPName]),

    {ok, _} = navstar_overlay_helper:run_command("ip route replace ~s/32 via ~s table 42", [FormattedAgentIP, FormattedVTEPIP]),
    {ok, _} = navstar_overlay_helper:run_command("ip route replace ~s/~B via ~s", [FormattedSubnetIP, SubnetPrefixLen, FormattedVTEPIP]).
